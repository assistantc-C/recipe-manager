...
YouTube の URL やスクショからレシピを自動入力できる、ローカル完結・PWA 対応の Web アプリ
...
- **🎬 YouTube の URL から自動入力** — 料理動画の URL を貼るだけ。AI が動画を視聴（映像・音声・テロップ）してタイトル・材料・手順を抽出（Gemini）
...
#### 🎬 YouTube の URL から自動入力（いちばん簡単）

1. 料理動画の URL をコピーして、URL 欄に貼り付け
2. 「🎬 この動画からレシピを自動入力」をタップ
3. AI が動画を視聴して、タイトル・材料・手順が自動入力（動画の長さにより数十秒〜1分ほど）

> Gemini の API キーが必要です（動画理解は Gemini のみ対応）。
> 非公開・限定公開の動画や、うまく抽出できない場合は、下の「画像から自動入力」をどうぞ。
...
| **YouTube URL（動画取り込み時）** | — | Google（Gemini）に送信 |

詳しくは [プライバシーポリシー](privacy-policy.md) を参照。

---

## 🤖 ちょい技：AI にレシピを活用してもらう

エクスポートした JSON ファイルを、そのまま AI（ChatGPT・Gemini・Claude）に渡してみてください。

> 「この中から今夜の献立を提案して」
> 「冷蔵庫に鶏肉とキャベツがある。作れるものは？」
> 「来週の平日5日分の献立と買い物リストにして」

データが JSON で持ち出せるアプリなので、献立提案・買い物リスト・栄養相談などは **お好みの AI に任せる**のがおすすめです。
...
- Google Gemini 動画理解（任意・YouTube URL からのレシピ抽出）

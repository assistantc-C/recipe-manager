...
- **🔗 共有から1タップ取り込み（Android/Chrome）** — PWA インストール後、共有メニューから直接レシピ追加
...
PWA をインストールすると、YouTube アプリやブラウザの「共有」メニューに「レシピ管理アプリ」が表示されます。共有 → 選択 → 自動的に追加モーダルが開いて URL が入力済みの状態になります。

> iOS / Safari は現状 Web Share Target API に未対応のため、本機能はご利用いただけません。
